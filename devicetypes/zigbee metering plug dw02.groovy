...
metadata {
    definition (name: "Zigbee Metering Plug DW02", namespace: "JiDo1517", author: "JiDo1517", ocfDeviceType: "oic.d.smartplug", mnmn: "SmartThings",  vid: "generic-switch-power-energy") {
        ...
        attribute "penergy", "number"
        attribute "tenergy", "number"
        command "reset"
    ...
    tiles(scale: 2){
        ...
        valueTile("tenergy", "device.tenergy", decoration: "flat", width: 2, height: 2) {
            state "default", label:'total ${currentValue} kWh'
        }
        standardTile("refresh", "device.power", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
            state "default", label:'', action:"refresh.refresh", icon:"st.secondary.refresh"
        }
        standardTile("reset", "device.penergy", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
            state "default", label:'reset kWh', action:"reset"
        }

        main(["switch"])
        details(["switch","power","tenergy","energy","refresh","reset"])
    }
}
    ...
    if (event) {
        ...
        attrData.each {
                ...
                if (it.value && it.clusterInt == zigbee.SIMPLE_METERING_CLUSTER && it.attrInt == ATTRIBUTE_HISTORICAL_CONSUMPTION) {
                        ...
                        map.unit = "kWh"
                        if (zigbee.convertHexToInt(it.value) == 0) {
                            def tempenergy = device.currentState("energy").value as float
                            sendEvent(name: "penergy", value: 0 - tempenergy, unit: "kWh", displayed: true)
                        }
                        def penergyValue = device.currentState("penergy").value as float
                        log.debug "tenergy : ${map.value} penergy: ${penergyValue}"
                        def energy = map.value - penergyValue
...
def reset() {
    log.debug "reset"
    def penergy = device.currentState("tenergy").value
    sendEvent(name: "penergy", value: penergy, unit: "kWh", displayed: true)
    sendEvent(name: "energy", value: 0, displayed: true)
}
